...
for train, test in outer_cv.split(X,y):
    ...
    clf = GridSearchCV(estimator=pipe, param_grid=param_grid1, cv=inner_cv, scoring='roc_auc', n_jobs=-1,refit=True)
    # clf.fit(X[train], y[train])
    nested_score = cross_val_score(clf, X[train], y[train], cv=outer_cv)
...
plt.title(
    "Nested Cross Validation on Iris Dataset",
    x=0.5,
    y=1.1,
    fontsize="15",
)
